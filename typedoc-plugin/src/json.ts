...
import type { DocumentationEntry, ParamSpecifier } from '@sourceacademy/documentation-types';
import * as td from 'typedoc';
import drawdown from './drawdown';

export interface ParserSuccess {
  obj: DocumentationEntry;
  warnings: string[];
}
...
export const parsers = {
  ...
  [K in td.ReflectionKind]?: (obj: td.DeclarationReflection) => DocumentationEntry
};

/**
 * Converts a Typedoc reflection into the format as expected by the frontend and write it to disk as a JSON file
 */
export function buildJson(reflection: td.ProjectReflection): Record<string, DocumentationEntry> {
  const jsonData = reflection.children!.reduce<Record<string, DocumentationEntry>>((res, element) => {
